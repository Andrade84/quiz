...
const questoes = [
  ...
  console.log("----- QUIZ DE FATOS ALEATÓRIOS -----");
  console.log("\n\n")
  console.log("Seja Bem-vindo jogador(a)!");
  const nome = entradaDados.question("Digite o seu nome: ")
  
  
  //Embaralhar as perguntas da coleção de dados aleatoriamente
...
//Função exibir uma pergunta por vez
...
function exibirPergunta(pergunta){
  ...
  if(respostaUsuario.trim().toLowerCase() === pergunta.resposta.toLowerCase()){
    console.log("Resposta correta!\n");
    return true;
  }else{
    console.log(`Resposta incorreta! A resposta correta é: ${pergunta.resposta}`)
    return false;
  }
}
...
for(let i = 0; i < perguntasSelecionadas.length; i++){
  console.log(" ");
  const perguntaAtual = perguntasSelecionadas[i];
  const numeroPergunta = i + 1;
  const respostaCorreta = exibirPergunta(perguntaAtual,numeroPergunta);
...
console.log(`\n${nome}, sua pontuação final é: ${pontuacao}/${quantidadePerguntas}`);